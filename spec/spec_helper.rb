...
def xml(name)
	...
	if File.exists?(file)
		prefix = %|<?xml version="1.0" encoding="utf-8" standalone="no"?><omnifocus xmlns="http://www.omnigroup.com/namespace/OmniFocus/v1">|
		suffix = "</omnifocus>"
		data = prefix + File.read(file) + suffix
		return Nokogiri::XML(data).at_xpath("/xmlns:omnifocus").children.first
	else
